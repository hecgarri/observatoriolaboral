...
for (i in 1:length(info)){
  info[[i]]$variables = mutate(info[[i]]$variables, sector = as.character(sector),
                               sector_ = ifelse(sector=="Comercio" | sector=="Hoteles y Restoranes", 
                                                "Comercio, Hoteles y Restoranes",
                                          ifelse(sector=="Administracion Publica" |
                                                 sector=="Construccion" |
                                                 ...
                                                 sector=="Mineria"|
                                                 sector=="Pesca"|
                                                 sector=="Transporte y Comunicaciones","Otros",sector)))
}
...
total.ocupados.nuble.sector.sexo =lapply(1:length(info), function(i) svyby(~I(cae_general=="Ocupado"),
                                                 by=~sector_+sexo, 
                                                subset(info[[i]], prov_e==84), svytotal,
                                                 multicore = TRUE, 
                                                 drop.empty.groups = FALSE, na.rm=TRUE) %>% mutate(mes=i))


total.ocupados.nuble.sector.sexo. =  data.table::rbindlist(total.ocupados.nuble.sector.sexo)

freq = list()
for (i in 1:length(info)){
  freq[[i]]= xtabs(~I(prov_e==84)+sector_+sexo, data=info[[i]]$variables) %>% data.frame() %>%
    filter(`I.prov_e....84.`== TRUE)
}

freq = data.table::rbindlist(freq)
...
total.ocupados.nuble.sector.sexo. = 
  total.ocupados.nuble.sector.sexo.[,c("sector_", "sexo", "I(prov_e == 84)TRUE", "se.I(prov_e == 84)TRUE","cv","frecuencia")]
